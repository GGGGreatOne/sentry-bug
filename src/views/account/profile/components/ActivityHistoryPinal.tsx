...
import ActivityHistoryCarItem from './ActivityHistoryCarItem'
...
function ActivityHistoryPinal({ value, index, isNomal, ...other }: TabPanelProps) {
  ...
  return (
    <Box
      ...
      {...other}
    >
      <FormControl sx={{ m: 1, minWidth: 80, ml: isLg ? 10 : 0 }}>
        <Select
      ...
      {!data?.list || data?.list.length === 0 ? (
        ...
        <ListContainer ref={listRef}>
          <Masonry columns={isMd ? 1 : isLg ? 2 : 3} spacing={16}>
            <>
              {data?.list.map((_, index) => (
                <ActivityHistoryCarItem
                  isNomal={isNomal}
                  key={_.boxId + index}
                  item={_}
                  index={index}
...
export default ActivityHistoryPinal
