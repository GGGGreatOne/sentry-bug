...
export interface Experiences extends ExperienceValueItem {
  id: number
}
...
const validationSchema = yup.object({
  data: yup.array().of(
    yup.object().shape({
      name: yup.string().required('Twitter Handle is required'),
...
const ExperienceForm = ({ value = { style: '0', experienceItem: [] }, onCancel, onHide }: Props) => {
  const isMd = useBreakpoint('md')
  const { updateBoxAboutExperienceCallback } = useEditBoxAboutData()
  const teamData: Experiences[] = useMemo(() => {
  ...
  const sortList = (values: any, data: Experiences[]) => {
  ...
  const handleSubmit = (values: any) => {
    const experienceList: ExperienceValueItem[] = []
    ...
    onHide ? onHide() : viewControl.hide('ExperienceDialog')
  }
  return (
    <Box>
            ...
            <Stack component={Form} spacing={16}>
                      ...
                      >
                        <Box sx={{ width: 48, height: 48 }}>
                          <FormItem name={`data[${index}].picture`}>
                            ...
                            >
                              ...
                              >
                                <ExperienceIcon style={{ margin: '0 auto', transform: isMd ? 'scale(.75)' : 'none' }} />
                              </Stack>
                            </Upload>
                          </FormItem>
                        </Box>
                        ...
                        <Stack gap={8} flexDirection={'column'} flex={1} paddingRight={30}>
                          <FormItem name={`data[${index}][name]`}>
                            <Input height={isMd ? 40 : 50} outlined value={value.name} placeholder="@Twitter Handle" />
                          </FormItem>
                          <FormItem name={`data[${index}][title]`}>
                            <Input height={isMd ? 40 : 50} outlined value={value.title} placeholder="Title(Optional)" />
                          </FormItem>
              ...
              >
                <Button
                  variant="outlined"
                  onClick={() => {
                    onCancel ? onCancel() : viewControl.hide('ExperienceDialog')
                  }}
                >
                  Cancel
                </Button>
