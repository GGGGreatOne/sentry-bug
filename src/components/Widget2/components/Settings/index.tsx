...
import { useIsMobile } from 'components/Widget2/hooks/useIsMobile'
...
export default function SettingsTab({
  ...
  const showDeadlineSettings = Boolean(chainId && !(L2_CHAIN_IDS as any).includes(chainId))
  ...
  return (
    <Menu ref={node}>
      <MenuButton
        disabled={
          !isChainSupported ||
          ((connectedChainId && chainId && chainId !== (connectedChainId as unknown as ChainId)) as boolean)
        }
        isActive={isOpen}
        compact={compact}
        onClick={toggleMenu}
