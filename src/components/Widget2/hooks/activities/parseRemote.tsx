...
import { currencyId } from 'components/Widget2/utils/currencyId'
import { NumberType, useFormatter } from 'components/Widget2/utils/formatNumbers'
import { isAddress } from 'utils'
import { isSameAddress } from 'components/Widget2/utils/isSameAddress'
...
function parseSwap(changes: TransactionChanges, formatNumberOrString: FormatNumberOrStringFunctionType) {
  if (changes.NftTransfer.length > 0 && changes.TokenTransfer.length === 1) {
    ...
    const title = changes.NftTransfer[0].direction === 'IN' ? `Bough` : `Sold`
...
function parseApprove(changes: TransactionChanges) {
  if (changes.TokenApproval.length === 1) {
    const title = parseInt(changes.TokenApproval[0].quantity) === 0 ? `Revoked Approval` : `Approved`
...
function parseSendReceive(
  ...
  if (changes.TokenTransfer.length === 2 && callsPositionManagerContract(assetActivity)) {
    return { title: `Removed Liquidity`, ...parseLPTransfers(changes, formatNumberOrString) }
  }
  ...
  if (transfer && assetName && amount) {
    ...
    if (transfer.direction === 'IN') {
      return isMoonpayPurchase && transfer.__typename === 'TokenTransfer'
        ? {
            title: `Purchased`,
            descriptor: `${amount} ${assetName} ${`for`} ${formatNumberOrString({
...
function parseMint(
  ...
  if (Object.keys(collectionMap).length === 1) {
    ...
    if (changes.TokenTransfer.length === 2 && callsPositionManagerContract(assetActivity)) {
      return { title: `Added Liquidity`, ...parseLPTransfers(changes, formatNumberOrString) }
    }
    return { title: `Minted`, descriptor: `${collectionMap[collectionName]} ${collectionName}` }
  }
  return { title: `Unknown Min` }
}
...
function parseUnknown(
  ...
  return { title: `Contract Interaction`, ...COMMON_CONTRACTS[assetActivity.details.to.toLowerCase()] }
}
