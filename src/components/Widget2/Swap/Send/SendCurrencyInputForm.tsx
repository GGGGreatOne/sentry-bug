...
import { ReverseArrow } from 'components/Widget2/components/Icons/ReverseArrow'
import { useSendContext } from 'components/Widget2/state/send/SendContext'
...
const InputErrorLookup = {
  [SendInputError.INSUFFICIENT_FUNDS]: <>Insufficient funds</>,
  [SendInputError.INSUFFICIENT_FUNDS_FOR_GAS]: <>Insufficient funds to cover network fee</>
}
...
export default function SendCurrencyInputForm({
  ...
  const fiatCurrencyEqualsTransferCurrency = !!inputCurrency && fiatCurrency && fiatCurrency.equals(inputCurrency)
  ...
  const handleSelectCurrency = useCallback(
    (currency: Currency) => {
      ...
      if (fiatCurrency && fiatCurrency.equals(currency)) {
        setSendState(prev => ({
          ...prev,
    ...
    <Wrapper $disabled={disabled}>
      <InputWrapper>
        ...
        <AlternateCurrencyDisplay
          disabled={fiatCurrencyEqualsTransferCurrency || false}
        ...
        <ClickableRowBetween onClick={() => setTokenSelectorOpen(true)}>
          <Row width="100%" gap="md">
            <CurrencySelectorRow width="100%" gap="md" onClick={() => setTokenSelectorOpen(true)}>
              {inputCurrency && (
                <CurrencyLogo currency={inputCurrency} size="36px" /*chainId={chainId ?? ChainId.MAINNET}*/ />
              )}
              <Row width="100%">
                <Column>
